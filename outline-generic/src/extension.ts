...
import { TreeExplorerNodeProvider, TreeExplorerNodeContentProvider, TreeExplorerNodeContent } from 'vscode';
...
class OutlineNodeProvider<T extends OutlineNode> implements TreeExplorerNodeProvider<T> {
  ...
  constructor(filePath: string) {
    this.contentProvider = new OutlineNodeContentProvider();
...
class OutlineNodeContentProvider<T extends OutlineNode> implements TreeExplorerNodeContentProvider<T> {
  provideNodeContent(node: T): TreeExplorerNodeContent {
    return {
      label: node.label,
      shouldInitiallyExpand: node.shouldInitiallyExpand,
      clickCommand: node.clickCommand
    };
  }
}
